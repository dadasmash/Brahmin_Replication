...
// -------------------------------------------------------------------------- //
// Rural-urban cleavages: all indicators
// -------------------------------------------------------------------------- //
...
qui{
	...
	* generate indicators
	foreach var in vote_rural_diff vote_rural_difc ineq_rural_qdif ineq_rural_diff{
		gen `var' = .
	}
	...
	*local y 2012
		...
		replace vote_rural_diff = _b[var] if year == `y'
		
		reg voteleft var i.inc i.educ i.agerec i.sex i.emp i.marital if year == `y' [pw=weight]
		replace vote_rural_difc = _b[var] if year == `y'
		...
		cap replace ineq_rural_qdif = _b[var] if year == `y'
		
		// Inequality: % income difference based on WID data
		cap reg log var if year == `y' [pw=weight]
		cap replace ineq_rural_diff = _b[var] if year == `y'
